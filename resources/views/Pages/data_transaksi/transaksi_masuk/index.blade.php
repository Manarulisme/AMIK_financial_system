...
<button type="button" class="btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#exampleModal">
                        ...
                        <tbody>
                          @forelse ($pemasukans as $pemasukan)
                            <tr>
                                ...
                                <td>
                                    {{ Carbon\Carbon::parse($pemasukan->tanggal)->translatedFormat('d F Y');  }}
                                </td>
                                <td>
                             {{ $pemasukan->name }}
                                </td>
                             ...
                             {{ number_format($pemasukan->nominal,0,',','.') }}
                                ...
                                <td class="text-center">
                                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('pemasukan.destroy', $pemasukan->id) }}" method="POST">
                                        <a href="{{ route('pemasukan.show', $pemasukan->id) }}" class="btn btn-sm btn-info"><i class="fa-solid fa-eye"></i></a>
                                        <a href="{{ route('pemasukan.edit', $pemasukan->id) }}" class="btn btn-sm btn-warning"><i class="fa-solid fa-gear"></i></a>
 ...
 <h3>
   Total: Rp.{{ number_format($pemasukans->pluck('nominal')->sum(),0,',','.')  }}
 </h3>
                </div>
            </div>
        </div>
